...
const { parseArgs, indexCompleted, buildPlan, statusForVersion, failedPhaseLines, resolveSchemaBehind } = __testing;
...
//
// FORK DELTA (0.42.57 → 0.45.13 merge): upstream asserts the INLINE loop it
// shipped for #921. This fork reached the same requirement first via the
// shared `failedPhaseLines` helper, which also covers the PARTIAL path
// upstream's inline version misses and is behaviour-tested directly in the
// `failedPhaseLines` describe above. The assertion is retargeted at that call
// site; the #921 requirement itself is unchanged.
describe('failed migration prints phase detail (#921)', () => {
  test('runner surfaces failed phase details on stderr at the failure announcement', async () => {
    const { readFileSync } = await import('fs');
    const src = readFileSync('src/commands/apply-migrations.ts', 'utf8');
    expect(src).toMatch(
      /reported status=failed[\s\S]{0,400}for \(const line of failedPhaseLines\(result\.phases\)\) console\.error\(line\)/,
    );
  });
});
