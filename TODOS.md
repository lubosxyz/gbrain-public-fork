...
## Tenant-token lane follow-ups (filed 2026-08-21, v0.46.12.4; capped-with-triage accepted P2/P3)

- [ ] **P3 — audit INSERT is not cancelled on timeout.** **What:** `writeAuthAudit` races the INSERT against a 2s timer and reports `ok:false` on timeout, but the SQL keeps running; a later-landing row is corrected by a superseding `deny/audit_timeout_superseded` row rather than cancelled. **Why:** the durable record never ends on a false allow, but two rows exist for one decision under a slow DB. **Fix:** thread a real statement-cancellation (pg `AbortSignal`/`statement_timeout`) so the INSERT is killed, not superseded. **Start:** `src/core/auth-audit.ts:writeAuthAudit`.
- [ ] **P3 — revocation is audited but not fail-closed, and multi-path.** **What:** CLI `auth revoke` (name + id), the `token_revoke` op, and the admin-console revoke endpoint all revoke and audit, but a failed audit does not un-revoke. **Why:** acceptable (revocation only narrows access), but the paths could converge on one audited primitive. **Start:** `src/commands/auth.ts`, `src/core/ops/auth-tokens.ts`, `src/commands/serve-http.ts` admin endpoint.
- [ ] **P3 — same-brain cross-source scoping unchanged.** **What:** cross-tenant isolation is structural only because one brain hosts one tenant (per-brain DB). Within a brain, a token with no explicit source grant can still select any `source_id`. **Why:** fine for the current per-brain deployment model; would become a tenant leak if multi-tenant-per-brain ever ships. **Start:** `src/core/ops/context.ts` source resolver + a per-brain-tenant assertion if that model changes.
