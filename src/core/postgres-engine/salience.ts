...
export async function getRecentSalience(deps: PgSalienceDeps, opts: SalienceOpts): Promise<SalienceResult[]> {
    ...
    const rows = await sql`
      SELECT p.slug, p.source_id, p.title, p.type, p.updated_at, ${sql.unsafe(emotionalSql)} AS emotional_weight, p.last_retrieved_at,
    ...
    return rows.map((r: Record<string, unknown>) => ({
      ...
      // Fork contract: expose the retrieval timestamp used by stale-page bias.
