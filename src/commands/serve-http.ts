...
<div class="prompt">&ldquo;Give me the GBrain admin login link&rdquo;</div>
  ...
  app.get('/admin/api/agents', requireAdmin, async (_req: Request, res: Response) => {
    try {
      ...
      // pre-v128 / failed-migration brain, referencing it in a scalar subquery
  ...
  app.post('/mcp', requireBearerAuth({ verifier: resourceVerifier, resourceMetadataUrl }), mcpJsonParser, async (req: Request, res: Response) => {
